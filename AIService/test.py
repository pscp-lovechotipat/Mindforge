...
def analyze_team_roles(
    ...
    for member_name, member_details in team_details.items():
        logger.info(f"Analyzing roles for team member: {member_name}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"Successfully analyzed roles for {member_name}")
                break
                
            except RoleAnalysisError as e:
                logger.warning(f"Attempt {attempt + 1}/{max_retries} failed for {member_name}: {e}")
                if attempt == max_retries - 1:
                    raise RoleAnalysisError(
                        f"Failed to analyze roles for {member_name} after {max_retries} attempts"
                    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        import asyncio
        async def main():
            async_result = await analyze_team_roles_async(team_details, llm)
            print("\nAsynchronous Results:")
            print(json.dumps(async_result, indent=2))
            
        asyncio.run(main())
        
    except (ValueError, RoleAnalysisError) as e:
